...
class TagController extends BasicController {
  async index() {
    ...
    try {
      const data = await ctx.service.tagService.list(query);
  ...
  async show() {
    const ctx = this.ctx;
    try {
      const data = await ctx.service.tagService.find(ctx.helper.parseInt(ctx.params.id));
      return this.send(data, 200, '获取成功');
    } catch (err) {
      return this.send(null, 500, err.message);
    }
  ...
  async create() {
    const ctx = this.ctx;
    try {
      const data = await ctx.service.tagService.create(ctx.request.body);
  ...
  async update() {
    ...
    try {
      const data = await ctx.service.tagService.update({ id, updates });
  ...
  async destroy() {
    ...
    try {
      await ctx.service.tagService.destroy({ id });
